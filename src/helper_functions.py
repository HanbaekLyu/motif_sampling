...
def Generate_corrupt_graph(path_load, path_save,
    ...
    if noise_type in ['ER_edges', 'WS', 'BA', 'lattice']:
        # edges are added
        edges_added = []
    ...
    elif noise_type in ['ER', '+ER']:
        #G_noise = nx.erdos_renyi_graph(noise_nodes, parameter)
        G_noise = nx.dense_gnm_random_graph(noise_nodes, parameter)  # num edges for second input
        G_new = nx.Graph(list(G.edges))

        deg_seq = [deg for n, deg in G.degree()]
        dist = deg_seq/np.sum(deg_seq)
        edges_false = []
        #noise_node_rate = 0.05
        #if len(node_list) > 10000:
        #    noise_node_rate = 0.05
        print("!!! ER noise nodes:", noise_nodes)
        node_list_sub = np.random.choice(node_list, noise_nodes)
        with tqdm.tqdm(total=len(edges_false)) as pbar:
            #G_noise = nx.erdos_renyi_graph(len(node_list_sub), parameter)
            nx.dense_gnm_random_graph(noise_nodes, parameter)  # num edges for second input
            #print("false edges to be added:", len(list(G_noise.edges)))
            for e in list(G_noise.edges):
                edge = node_list_sub[e[0]], node_list_sub[e[1]]
                G_new.add_edge(edge[0], edge[1])
                edges_false.append(edge )
            """
            while(len(edges_false) < int(parameter*len(edgelist))):
                #e = np.random.choice(node_list, 2, p = dist, replace=False)
                e = np.random.choice(node_list, 2, replace=False)
                e = (str(e[0]), str(e[1]))
                #e = (str(u), str(v))
                if e[0] >= e[1]:
                    continue
                ...
                if e in original_edges or (e[1],e[0]) in original_edges:
                    continue
                edges_false.append(e)
                pbar.update(1)
                G_new.add_edge(e[0], e[1])
            """
        edges_changed = [e for e in list(G_new.edges) if not G.has_edge(e[0],e[1])]
        print('edges added = {}'.format(len(edges_changed)))
        ...
        if len(node_list) > 10000:
            noise_node_rate = 0.01
        node_list_sub = np.random.choice(node_list, int(len(node_list)*noise_node_rate))#, p=dist)
        """

        #G_original_sub = G_original.subgraph(node_list_sub)
        ...
        """
        edges_deleted = []
        with tqdm.tqdm(total=len(edges_deleted)) as pbar:
            for e in edges_non_mst:
            #while(len(edges_deleted) < int(0.1*len(edges_non_mst))):
                #e = np.random.choice(node_list_sub, 2, p = dist, replace=False)
                #e = np.random.choice(node_list_sub, 2, replace=False)
                e = (str(e[0]), str(e[1]))
                #e = (str(u), str(v))
                if e[0] >= e[1]:
                    continue
                if e in edges_deleted:
                    continue
                if e in mst_edgelist: # or (e[1],e[0]) not in original_edges:
                    continue
                if np.random.rand() < 1:
                    edges_deleted.append(e)
                    pbar.update(1)
                    G_new.remove_edge(e[0], e[1])
        edges_changed = edges_deleted
        print('edges deleted = {}'.format(len(edges_changed)))
        """
        G_new = nx.Graph(mst_edgelist)
        idx_array = np.random.choice(range(len(edges_non_mst)), int(len(edges_non_mst)*0.5), replace=False)
...
def compute_ROC_AUC_old(G_original=None,
                    recons_wtd_edgelist=[],
                    is_dict_edges=False,
                    path_original=None,
                    path_corrupt=None,
    ...
    recons_wtd_edgelist = G_recons.get_wtd_edgelist()
    print('num edges in G_recons', len(recons_wtd_edgelist))
    # print('wtd edges in G_recons', G_recons.get_wtd_edgelist())
    print("example edges in G_original:", recons_wtd_edgelist[0])
    ...
    print("example edges in G_original:", edgelist_original[0])
...
def compute_ROC_AUC(G_original=None,
    ...
    if G_corrupted is None:
        ...
        G_corrupted.load_add_edges(path_corrupt, increment_weights=False, delimiter=delimiter_corrupt,
                                       use_genfromtxt=True)
    edgelist_full = G_corrupted.get_wtd_edgelist()
    # else G_corrupted is given as a Wtd_netwk form
    print('num edges in G_corrupted', len(G_corrupted.get_wtd_edgelist()))
